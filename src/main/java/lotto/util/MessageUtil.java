...
public class MessageUtil {
    ...
    public void printWinningStatsResult(int number, int price, int matchCount) {
        ...
        if (number == FIVE_COUNT_WITH_BONUS.getNumber()) {
            System.out.printf(WINNING_STATS_BONUS_RESULT.getValue(), priceWithComma, matchCount);
            return;
        }
        System.out.printf(WINNING_STATS_RESULT.getValue(), number, priceWithComma, matchCount);
    }

    public void printEarningRate(Double rate) {
        System.out.printf(EARNING_RATE.getValue(), rate);
    }
}
